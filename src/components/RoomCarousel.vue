<template>
  <div v-if="imgfullScreen">
    <teleport to="body">
      <div class="album-mask absolute left-0 top-0 h-full w-full" @click="toggleImgFullScreen">
        <swiper :modules="modules" :navigation="{ nextEl: '.next-btn', prevEl: '.prev-btn', clickable: true }"
          :speed="800">
          ...
          <div class="prev-btn absolute left-32 top-[50%] z-50 cursor-pointer hover:scale-110" @click.stop>
            <img src="../assets/images/albumleft.svg" alt="albumArrow" />
          </div>
          <div class="next-btn absolute right-32 top-[50%] z-50 cursor-pointer hover:scale-110" @click.stop>
  ...
  <div v-else class="relative">
    <swiper :modules="modules" :pagination="{ el: '.swiper-pagination', type: 'bullets', clickable: true }"
      :autoplay="{ delay: 2000 }" :speed="1000" :loop="true" class="h-screen">
      <swiper-slide v-for="(url, index) in props.room.imageUrl" :key="index">
    ...
    <div class="absolute bottom-0 left-0 right-0 top-0 z-10 flex cursor-pointer flex-col" @click="toggleImgFullScreen">
      <div
      ...
      <div class="mx-auto mt-[556px] max-w-[44%]">
        <p class="mb-3 text-center text-4xl text-primary">
          ${{ bookingDate.totalPrice }} <span class="ml-3 text-lg text-primary lg:text-xl">／</span><span
            class="ml-4 text-xl text-primary">{{ bookingDate.totalNights }}晚</span>
        </p>
        <button
